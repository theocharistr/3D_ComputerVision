...
{
	if (argc < 3) {
		std::cerr << "Usage: " << argv[0] << " input.xyz output.xyz\n" ;
		return 1;
	}
	MatrixReaderWriter* mrw;
	...
	FitPlaneRANSAC(
		*mrw,
		...
		0.05, // The inlier-outlier threshold 0.05 to 0.1
	...
	printf("The plane's coefficients of RANSAC are a = %f, b = %f, c = %f, d = %f \n", a1, b1, c1, d1);
	...
	printf("The plane's coefficients of LSQ are a = %f, b = %f, c = %f, d = %f \n", a2, b2, c2, d2);
	...
	printf("Average RANSAC error is %f px\n", averageError1);
	printf("Average LSQ error is %f px\n", averageError2);
